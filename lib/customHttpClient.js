...
class CustomHttpClient {
	constructor(param, buff) {
		...
		this.init();
		this.myrng = seedrandom(String(this.id));
	}
	init() {
	...
	randomExponential(rate, randomUniform) {
 		...
 		if (typeof randomUniform === 'function') U = randomUniform();
  		...
  		//log.info('thread id:' + this.id + ' random number is:' + U);
  		return -Math.log(U)/rate;
	}

	warmup() {
		// send 20s requests to warmup loadtest
		const rps = 5;
		const warmup_duration = 20; // 20 seconds
		const total_requests = rps * warmup_duration;
		var cumulative_time = 0;
                for (var i = 0; i < total_requests; i++) {
                	var interval = warmup_duration * 1000 * this.randomExponential(total_requests);
			cumulative_time += interval;
                        setTimeout(() => this.makeRequest(), cumulative_time);
			//log.info('id ' + this.id + 'interval is ' + interval + ' total requests ' + total_requests);
                }
	}
	start() {
		log.info('id ' + this.id + ' start...');
		let index = 0;
		if (this.params.rpsInterval) {
			this.rpsIntervalTimer = new HighResolutionTimer(this.params.rpsInterval * 1000, () => {
				...
				var cumulative_time = 0;
				for (var i = 0; i < rps; i++) {
					var interval = this.params.rpsInterval * 1000 * this.randomExponential(rps);
					cumulative_time += interval;
					setTimeout(() => this.makeRequest(), cumulative_time); 	
					//log.info('xiaosu:' + interval);
				}
				log.info('xiaosu total time:' + cumulative_time);
...
parentPort.on("message", message => {
	...
	client.warmup();
	//wait for 21 seconds then calling start()
	setTimeout(() => client.start(), 21 * 1000);
});
